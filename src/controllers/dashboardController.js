...
/** วันที่ตาม UTC สำหรับจัดกลุ่มต่อวัน YYYY-MM-DD */
...
/** ดึงทุกแถวแบบแบ่งหน้า (ใช้ aggregate ใน memory — เหมาะกับข้อมูลปานกลาง) */
async function selectAllPaged(buildSelect, pageSize = 2000) {
  const rows = [];
  let offset = 0;
  for (;;) {
    const { data, error } = await buildSelect().range(offset, offset + pageSize - 1);
    if (error) throw error;
    const chunk = data || [];
    rows.push(...chunk);
...
const overview = async (req, res) => {
  try {
    ...
    /** @type {Map<number, { time: number, at: string }>} */
    const latestSubmissionByUser = new Map();
    /** @type {Map<number, { total_attempt: number, total_unfinished: number, total_finished: number, submissions_passed: number, submissions_not_passed: number, questionIds: Set<number>, byDay: Map<string, number> }>} */
    ...
    const submissionRows = await selectAllPaged(
      () => from(TABLE_NAMES.SUBMISSIONS).select('user_id, question_id, status, created_at'),
      2000
    );
    ...
    for (const row of submissionRows) {
      ...
      if (row.user_id != null && row.created_at) {
        ...
        if (!prev || t > prev.time) {
          latestSubmissionByUser.set(row.user_id, { time: t, at: new Date(row.created_at).toISOString() });
        }
      }
      if (row.user_id != null) {
    ...
    if (topFiveIds.length > 0) {
      ...
      topQuestions = topFiveIds.map((id) => {
        const q = byId.get(id);
        return {
          question_id: id,
          code: q?.code ?? null,
