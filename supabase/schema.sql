...
CREATE OR REPLACE FUNCTION "public"."leaderboard_top_100"() RETURNS TABLE("rank" bigint, "user_id" bigint, "total_point" integer, "solved_count" bigint, "display_name" character varying, "email" character varying, "avatar_url" text)
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
  WITH scores AS (
...
-- # เพิ่มฟิลด์ข้อมูลส่วนตัวเพิ่มเติมให้กับตารางผู้ใช้
...
-- # ตารางเก็บข้อมูลการใช้งานต่อเนื่องของผู้ใช้ (Streaks)
-- # User ID -> Current/Max Streak -> Activity History
...
-- # ตารางหลักสำหรับเก็บข้อมูลความสำเร็จ (Achievements) และเงื่อนไขการได้รับ
-- # Name/Icon -> Criteria Type -> Color
...
-- # 5. Audit Logs Table
-- # ตารางเก็บประวัติการใช้งานและกิจกรรมสำคัญในระบบเพื่อการตรวจสอบ
-- # User ID -> Action Type -> Details -> IP/UserAgent
...
-- # Security Setup (RLS)
-- # เปิดใช้งานระบบความปลอดภัยระดับแถว (Row Level Security) และตั้งค่า Policies
...
-- # ข้อมูลเริ่มต้นสำหรับระบบความสำเร็จ
